...
- **场景预设（分类讨论）**：极限文本 / 理科公式 / 图文混排 / 均衡默认 四类，按内容特征（正文字数、独立公式密度、图片块占比）自动推荐、用户可改——背诵型大文本才走极致压缩，公式课公式绝不缩小（宁可升宽档）
...
| 接口 | 说明 |
|------|------|
| `POST /api/scene` | **场景排版一站式接口（网格引擎）**：内容统计 → 场景推荐（或用户指定）→ 公式预检 → 字号搜索 → 渲染，返回统计/推荐理由/警告/`jobId`。图片以 data: URI 内嵌在 Markdown 里 |
