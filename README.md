...
`scene-presets.ts` 是它的粗糙前身，代码按该规范逐步收敛。
...
| 接口 | 说明 |
|------|------|
| `POST /api/scene` | **场景排版一站式接口（网格引擎）**：内容统计 → 场景推荐（或用户指定）→ 公式预检 → 字号搜索 → 渲染，返回统计/推荐理由/警告/`jobId`。图片以 data: URI 内嵌在 Markdown 里；`debug: true` 叠加网格调试层（文件名加「-网格」后缀，不覆盖正式版）；`allowReorder: true` 声明内容可乱序（RULES.md S2），开启跨页回填换密度 |
